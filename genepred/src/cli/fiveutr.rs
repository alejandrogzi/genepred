...
//! `genepred fiveutr` subcommand: emit 5' UTR intervals (strand-aware) as BED rows.
...
/// Runs the `fiveutr` subcommand against `path`, writing BED rows to `writer`.
pub fn run<P, W>(
    path: P,
    writer: &mut W,
...
where
    ...
    W: Write + ?Sized,
{
    options.kind = FeatureKind::FivePrimeUtr;
