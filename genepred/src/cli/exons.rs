...
//! `genepred exons` subcommand: emit each exon interval as a BED row.
...
/// Runs the `exons` subcommand against `path`, writing BED rows to `writer`.
pub fn run<P, W>(
    path: P,
    writer: &mut W,
...
where
    ...
    W: Write + ?Sized,
{
    options.kind = FeatureKind::Exons;
